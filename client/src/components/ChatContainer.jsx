...
function ChatContainer() {
  ...
  if (!selectedUser) {
    return (
      <div className="hidden md:flex flex-col gap-4 items-center justify-center h-full text-center px-8 bg-[#0a0918]/40">
        <div className="w-20 h-20 rounded-full bg-violet-500/10 flex items-center justify-center">
          <img src={assets.logo_icon} alt="" className="w-10 opacity-50" />
        </div>
        <div>
          <p className="text-base font-semibold text-white">Your messages</p>
          <p className="text-sm text-gray-500 mt-1 max-w-[200px]">Select a conversation to start chatting</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0918]/40 overflow-hidden">

      {/* ── Header ── */}
      <div className="flex-shrink-0 flex items-center gap-3 px-4 py-3 border-b border-white/5 bg-[#0f0e1a]/60 pt-safe">
        <button
      ...
      {/* ── Messages ── */}
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1" role="log" aria-live="polite" aria-label="Messages">
        {messages.length === 0 && (
      ...
      {/* ── Input bar ── */}
      <div className="flex-shrink-0 px-3 pt-2 pb-3 pb-safe border-t border-white/5 bg-[#0f0e1a]/60">
        <form onSubmit={handleSend} className="flex items-center gap-2">
          <div className="flex-1 flex items-center gap-2 bg-white/6 border border-white/10 rounded-full px-4 focus-within:border-violet-500/50 transition min-w-0">
            <input
              ref={inputRef}
              value={input}
              ...
              className="flex-1 py-3 bg-transparent outline-none text-white text-base placeholder-gray-600 min-w-0"
            />
            <input onChange={handleImage} type="file" id="chat-image" accept="image/png,image/jpeg,image/webp" hidden />
            <label htmlFor="chat-image" className="flex-shrink-0 cursor-pointer p-1 text-gray-500 hover:text-gray-300 active:text-gray-200 transition" aria-label="Attach image">
          ...
          <button
            ...
            className="w-11 h-11 flex-shrink-0 flex items-center justify-center bg-violet-600 hover:bg-violet-500 active:bg-violet-700 disabled:opacity-30 disabled:cursor-not-allowed rounded-full transition-all duration-150 active:scale-95"
          >
